...
class AutoGeneratedRestClientTest {
    ...
    @Test void simpleGetWithURI()
    {
        ...
        val response = await( client.execute(
            GET, extraBaseURI,
    ...
    @Test void simpleGetWithHeaders()
    {
        ...
        val response = await( client.execute(GET, "", headers, EchoResponse.class) );
...
//        val conf = newConf("http://localhost:8080").build();
        val client = new TestableClient(conf);

        val response = await( client.send(
            POST, "post",
            MultiMap.caseInsensitiveMultiMap(),
